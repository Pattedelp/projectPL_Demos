...
function Productos() {
  ...
  async function obtenerTodo() {
    const { data: dataProductos, error } = await supabase
      .from("productos")
      .select(`*, categorias(nombre), producto_proveedores(proveedor_id), stock_sucursal!inner(stock, stock_minimo, sucursal_id)`)
      .eq("negocio_id", negocio.id)
      .eq("stock_sucursal.sucursal_id", sucursalActual.id)
      .order("created_at", { ascending: false })

    const { data: dataCategorias } = await supabase
      .from("categorias").select("*").eq("negocio_id", negocio.id).order("nombre")

    const { data: dataProveedores } = await supabase
      .from("proveedores").select("*").eq("negocio_id", negocio.id).order("nombre")
  ...
  async function crearCategoria() {
    ...
    const { data, error } = await supabase
      ...
      .select().single()
    if (error) { alert("Error al crear la categoría"); return }
  ...
  async function handleSubmit(e) {
    ...
    if (editandoId) {
      ...
      ;({ error } = await supabase.from("productos").update(payload).eq("id", editandoId))
    } else {
      const { data, error: errorInsert } = await supabase.from("productos").insert([payload]).select().single()
      error = errorInsert
      productoId = data?.id
    }
    if (error) { alert("Hubo un error al guardar el producto"); setGuardando(false); return }

    await supabase.from("producto_proveedores").delete().eq("producto_id", productoId)
    if (proveedoresSeleccionados.length > 0) {
      await supabase.from("producto_proveedores").insert(
        proveedoresSeleccionados.map((proveedor_id) => ({ producto_id: productoId, proveedor_id }))
      )
    }
  ...
  function toggleCategoria(nombreCategoria) {
    setCategoriasExpandidas((prev) => ({ ...prev, [nombreCategoria]: !prev[nombreCategoria] }))
  ...
  function cargarMas(nombreCategoria) {
    setCantidadVisible((prev) => ({ ...prev, [nombreCategoria]: (prev[nombreCategoria] || 12) + 12 }))
  }
  ...
  function obtenerResumenStockBajo() {
    ...
    productosStockBajo.forEach((p) => {
      const proveedorIds = p.producto_proveedores?.map((pp) => pp.proveedor_id) || []
      if (proveedorIds.length === 0) return
      const primerProveedorId = proveedorIds[0]
      if (!porProveedor[primerProveedorId]) porProveedor[primerProveedorId] = []
  ...
  function contactarProveedor(proveedorId, productosDelProveedor) {
    ...
    if (!proveedor) return
    const mensaje = armarMensajeProveedor(proveedor.nombre, productosDelProveedor)
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Productos / Stock</h1>
          <p className="text-muted-foreground mt-1">Inventario del negocio.</p>
        </div>
        <Dialog open={open} onOpenChange={(value) => {
          setOpen(value)
          if (!value) { setEditandoId(null); setForm(FORM_VACIO); setCreandoCategoria(false); setProveedoresSeleccionados([]) }
        }}>
          <DialogTrigger asChild>
            <Button onClick={abrirNuevo}><Plus size={18} className="mr-2" />Nuevo producto</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-2">
              ...
              <div>
                ...
                {!creandoCategoria ? (
                  <div className="flex gap-2 mt-1">
                    <select id="categoria" name="categoria_id" value={form.categoria_id} onChange={handleChange}
                    ...
                    <button type="button" onClick={() => setCreandoCategoria(true)}
                  ...
                  <div className="flex gap-2 mt-1">
                    <Input autoFocus placeholder="Nombre de la categoría" value={nombreNuevaCategoria}
                    ...
                    <button type="button" onClick={() => { setCreandoCategoria(false); setNombreNuevaCategoria("") }}
                      className="text-muted-foreground hover:text-foreground text-sm px-2">Cancelar</button>
                  </div>
                )}
              </div>
              <div>
                <Label className="mb-2 block">
                  ...
                  {proveedoresSeleccionados.length > 0 && (
                    <span className="text-muted-foreground font-normal"> ({proveedoresSeleccionados.length} seleccionado{proveedoresSeleccionados.length > 1 ? "s" : ""})</span>
                  )}
                </Label>
                {proveedores.length === 0 ? (
                  <p className="text-muted-foreground text-xs">No tenés proveedores cargados todavía.</p>
                ) : (
                  <>
                    {proveedores.length > 6 && (
                      <Input placeholder="Buscar proveedor..." value={busquedaProveedor}
                        onChange={(e) => setBusquedaProveedor(e.target.value)} className="mb-2 h-8" />
                    )}
                    <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                      {proveedoresFiltrados.map((prov) => {
                        const seleccionado = proveedoresSeleccionados.includes(prov.id)
                        return (
              ...
              <div className="grid grid-cols-3 gap-3">
                <div><Label htmlFor="precio">Precio</Label><InputPrecio id="precio" name="precio" value={form.precio} onChange={handleChange} required /></div>
                <div><Label htmlFor="stock">Stock</Label><Input id="stock" name="stock" type="number" value={form.stock} onChange={handleChange} required /></div>
                <div><Label htmlFor="stock_minimo">Mínimo</Label><Input id="stock_minimo" name="stock_minimo" type="number" value={form.stock_minimo} onChange={handleChange} /></div>
              </div>
      ...
      <Input placeholder="Buscar producto por nombre..." value={busqueda}
        onChange={(e) => setBusqueda(e.target.value)} className="mb-4 max-w-md" />
        ...
        <div className="bg-card border border-amber-500/30 rounded-lg p-4 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-foreground font-medium text-sm">
              Tenés stock bajo en {proveedorIdsConPendientes.length} proveedor(es)
            </h3>
          </div>
          <div className="flex flex-wrap gap-2">
            {proveedorIdsConPendientes.map((proveedorId) => {
              ...
              return (
                <button key={proveedorId} onClick={() => contactarProveedor(proveedorId, resumenStockBajo[proveedorId])}
                  ...
                  <MessageCircle size={12} />{proveedor?.nombre} ({cant})
                </button>
              )
            })}
          </div>
      ...
      {cargando ? (
        ...
        <div className="space-y-3">
          {Object.entries(grupos).map(([nombreCategoria, items]) => {
            ...
            return (
              <div key={nombreCategoria} className="bg-card border border-border rounded-lg overflow-hidden">
                <button onClick={() => toggleCategoria(nombreCategoria)}
                ...
                {expandida && (
                  <div className="p-4 pt-0">
                      ...
                      {itemsVisibles.map((producto) => {
                        ...
                        return (
                          <div key={producto.id}
                            ...
                            <h3 className="text-foreground font-semibold pr-6">{producto.nombre}</h3>
                            {stockBajo && (
                            ...
                            {producto.descripcion && (
                              <p className="text-muted-foreground text-sm mb-3 line-clamp-2 mt-1">{producto.descripcion}</p>
                            )}
                            <div className="flex items-center justify-between pt-3 border-t border-border mt-2">
                              <span className="text-foreground font-semibold">${Number(producto.precio).toLocaleString("es-AR")}</span>
                              <span className={`text-sm ${stockBajo ? "text-red-400" : "text-muted-foreground"}`}>Stock: {producto.stock}</span>
                            </div>
                            {stockBajo && producto.producto_proveedores?.length > 0 && (
                              <button onClick={() => {
                                const proveedor = proveedores.find((p) => p.id === producto.producto_proveedores[0].proveedor_id)
                                if (!proveedor) return
                                window.open(generarLinkWhatsApp(proveedor.telefono, armarMensajeProveedor(proveedor.nombre, [producto])), "_blank")
                              }}
                                ...
                                <MessageCircle size={12} />Pedir reposición
                              </button>
                            )}
                          </div>
                        )
                    ...
                    {hayMas && <ObservadorScroll onVisible={() => cargarMas(nombreCategoria)} />}
                  </div>
                )}
              </div>
            )
